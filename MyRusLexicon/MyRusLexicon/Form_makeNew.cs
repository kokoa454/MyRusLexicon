...
namespace MyRusLexicon
{
    public partial class Form_makeNew : Form
    {
        ...
        private void button_OK_Click(object sender, EventArgs e)
        {
            ...
            string partOFSpeach = richTextBox_partOfSpeech.Text;
            ...
            if (string.IsNullOrEmpty(word) || string.IsNullOrEmpty(translation) || string.IsNullOrEmpty(partOFSpeach))
            {
                MessageBox.Show("単語名・訳・品詞名は必須項目です。", "エラー", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }

            dbHelper.addWord(word, translation, partOFSpeach, exampleSentence1, exampleSentenceTranslation1, exampleSentence2, exampleSentenceTranslation2);

            
            ListViewItem item = new ListViewItem(word);
            item.SubItems.Add(partOFSpeach);
